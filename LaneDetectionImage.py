...
def pipeline(image):
    ...
    lower_yellow = np.array([ 25, 126, 190])#rgb
    ...
    if DEBUG:
        cv2.imshow("mask_white_yellow",mask_white_yellow)
    ...
    if DEBUG:
        cv2.imshow("combined_L1",combined_L1)
    ...
    if DEBUG:
        cv2.imshow('v_binary_hsv',v_binary_hsv)
    ...
    if DEBUG:
        cv2.imshow("combined_L1_V",combined_L1_V)
...
pt1 = (0,img_size[0])
pt2 = (int(img_size[1] * .37), int(img_size[0]/1.8))
pt3 = (int(img_size[1] * .63), int(img_size[0]/1.8))
pt4 = (int(img_size[1]) , int(img_size[0]))
...
image = mpimg.imread('test_images/Untitled.jpg')
