...
s1 = sock.connect(('192.168.1.180', 15001))
            ...
            if len(faces)==1:
                for (x, y, w, h) in faces:
                    ...
                    for path in image_paths:
                        ...
                        crop_image = np.array(predict_image_pil, 'uint8')
                        nbr_predicted = recognizer.predict(crop_image[y: y + h, x: x + w])
                        ...
                        if nbr_actual == nbr_predicted:
                            print "images are same  ".format(nbr_actual,nbr_predicted)
                            #sock.send("stop")
                            #time.sleep(2)
                            data_send = "STOPBOT   "
                            flag = 1
