...
def get_images_and_labels(imagepath): 
    ...
    for image_path in image_paths:
        ...
        nbr = int(os.path.split(image_path)[1].split(".")[0].replace("priya", ""))
    ...
    for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
        ...
        if len(faces)>=1:
           for (x, y, w, h) in faces:
               ...
               #print "after predicted image"
           
                  #faces1 = faceCascade.detectMultiScale(predict_image)
           #for (x, y, w, h) in faces:
                for path in image_paths:
                    #print path
                    nbr_predicted, conf = recognizer.predict(crop_image1)
                    print "nbr_predicted ",nbr_predicted
                    nx = (os.path.split(path)[1].split(".")[0].replace("priya", ""))
                    print nx
                    nbr_actual =  int(os.path.split(path)[1].split(".")[0].replace("priya", ""))
                    print "nbr_actual ", nbr_actual
                # print "atfer face detection of  predicted image and conf is {}  "
                    if nbr_actual == nbr_predicted:
                        print "Image matches with databse "
                        print " this is priya"
                        print  (os.path.split(path)[1].split(".")[0].replace("priya", ""))
                      print conf
