...
##        self.rpm= self.stab[2].split("=")
##        self.speed= self.stab[0].split("=")
##        self.accel= self.stab[1].split("=")
##        self.throt= self.stab[5].split("=")
        self.key2= self.stab[2].split("=")
        self.LATI= self.stab[1].split("=")
        self.LONGI= self.stab[0].split("=")
##        self.ALTI= self.stab[0].split("=")
        self._set_headers()
...
##        print 'ALTI {}'.format(self.ALTI)
        if(self.key == self.key2[1]) :    
            try:
##                sql= 'INSERT INTO para(RPM,SPEED,ACCEL,THROT)VALUES(%s,%s,%s,%s)'%(self.rpm[1],self.speed[1],self.accel[1],self.throt[1])
##                cursor.execute(sql)
##                db.commit()
