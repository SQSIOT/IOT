    ...
    for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
        ...
        if len(faces)>=1:
           ...
           #cv2.imshow("Faces found" ,image1)
                ...
                nbr_predicted, conf = recognizer.predict(crop_image[y: y + h, x: x + w])           
                nbr_actual =  int(os.path.split(path)[1].split(".")[0].replace("image", "")) 
                ...
                if nbr_actual == nbr_predicted:
                   ...
                   #print "both are different  ".format(nbr_actual,nbr_predicted)
