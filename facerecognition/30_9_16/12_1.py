...
imagepath = "/home/pi/FaceDetection/Database/images"
...
try:
    ...
    for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
        ...
        if len(faces)==1:
           ...
           #cv2.imwrite(file,gray)                          # to save the image
           ...
           for (x, y, w, h) in faces:
		       # predicate image -> camera captured image
               nbr_predicted, conf = recognizer.predict(predict_image[y: y + h, x: x + w]) 
			   # actual image -> images from database
               nbr_actual =  int(os.path.split(path)[1].split(".")[0].replace("image", "")) 
               print "atfer face detection of  predicted image and conf is ",conf
