...
file = "./database/subject"+str(i)+".png"    # path of database
...
##                    retValue = True
                    ...
                    if (conf>= 100  and conf <=130):
                        print "{} is Correctly Recognized with confidence {}".format(nbr_actual, conf)
                        ret = True
                        retValue = True

                    elif (conf > 130 and conf <= 165):
                        print "{} is Correctly Recognized with confidence {}".format(nbr_actual, conf)
                        ret = True
                        retValue = True
                    elif (conf >= 175 ):
...
####    Sign = "20"
    ...
    for frame in camera.capture_continuous(rawCapture, format="rgb", use_video_port=True):
        ...
        if (fSignalDetected != True):  #if no signal detected in previous frame then try to detect signal in present frame
            ...
            damm_1 = IsFaceDetectedInFrame(frame ,fFaceDetected)
            fFaceDetected = damm_1[0]
            ret= damm_1[1]
            if (fFaceDetected):
...
##                data1 = str('0')+str(MusicNumber) #Send message to start music
                ...
                elif (cnt == '1' and ((ret == False))):
                    Sign = "02"
                    time.sleep(1.8)
                    cnt = '2'
##                    music_two = True
                    Sign = "20" #Send message to start BOT
                elif (cnt == '2' and (ret == True)):  
                    Sign = "03"
                    time.sleep(1.8)
                    cnt = '0'
##                    music_two = True
